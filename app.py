...
async def get_questions(query: str):
    ...
    try:
        questions_data = await manager.get_clarifying_questions(query)
        questions_markdown = "## Clarifying Questions\n\n"
        ...
        yield questions_markdown
    except Exception as e:
        yield f"Error generating questions: {str(e)}"


async def run(query: str, send_email: bool, recipient_email: str, answer1: str, answer2: str, answer3: str):
    ...
    async for chunk in ResearchManager().run(query, send_email, recipient_email if recipient_email else None, answers):
        yield chunk
...
with gr.Blocks(theme=gr.themes.Monochrome()) as ui:
    ...
    def show_questions(questions_output):
        ...
        if questions_output and "Clarifying Questions" in questions_output:
            return [
                gr.update(visible=True),  # questions_display
    ...
    run_button.click(
        fn=run,
        inputs=[query_textbox, send_email_checkbox, recipient_email_textbox, answer1_textbox, answer2_textbox, answer3_textbox],
        outputs=report
    )
